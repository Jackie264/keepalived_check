...
# Check if this is the first run since system startup
if [ ! -f /tmp/keepalived_check_initialized ]; then
        sleep 90
        touch /tmp/keepalived_check_initialized
fi
...
check_keepalived_state() {
        state=$(cat /tmp/keepalived_state 2>/dev/null)
        if [ -z "$state" ]; then
                echo "Error: Unable to determine Keepalived state from /tmp/keepalived_state"
                logger "Error: Unable to determine Keepalived state from /tmp/keepalived_state"
                return 1
        fi
